								...
								<table class="responsive-table striped" id="myjobs">
									...
									<tbody id="results">
										...
										?>
											...
											<tr>
												<td>
													<a href="<?php echo ($publishcount > 0)? $this->config->base_url().'Profile/List/'.$result->job_url_id : $this->config->base_url().'Jobs/Viewdetails/'.$result->job_url_id ?>"> <?php echo $result->job_title;?> </a>
												</td>
												<td>
													<a href="<?php echo ($publishcount > 0)? $this->config->base_url().'Profile/List/'.$result->job_url_id : $this->config->base_url().'Jobs/Viewdetails/'.$result->job_url_id ?>"><?php echo $result->jfun_display;?></a>
												</td>
												<td>
													<a href="<?php echo ($publishcount > 0)? $this->config->base_url().'Profile/List/'.$result->job_url_id : $this->config->base_url().'Jobs/Viewdetails/'.$result->job_url_id ?>"><?php echo $result->job_location;?></a>
												</td>
												<td>
													<a href="<?php echo ($publishcount > 0)? $this->config->base_url().'Profile/List/'.$result->job_url_id : $this->config->base_url().'Jobs/Viewdetails/'.$result->job_url_id ?>"><?php echo date('d M Y', strtotime($result->job_created_dt));?></a>
												</td>
												<td>
													<a href="<?php echo $this->config->base_url().'Jobs/Reopen/'.$result->job_url_id;?>" class="reopen">
														<i class="material-icons brand-text ">file_upload</i> Re Open
													
													
												</td>
												<td>
													<a href="<?php echo $this->config->base_url().'Jobs/Delete/'. $result->job_url_id; ?>" class="del-jobs">
							...
							<div class="card">
								<div class="card-content">
									<div class="center ptb30">
										...
										<br>
										<a href="" class="btn brand waves-effect">Add New Jobs</a>
									</div>
								</div>
...
<script type="text/javascript" src="<?php echo $this->config->item('web_url')?>assets/js/daterangepicker.js"></script>
	...
	$(document).ready(function(){
	    ...
	    $('select').formSelect();
	    $('.del-jobs').click(function(){if (confirm('Do you really want to delete this vacancy?') == true) {return true; } else{return false; } });
	    $('.reopen').click(function(){if (confirm('Do you really want to re-open this vacancy?') == true) {return true; } else{return false; } });
	    /* ajax date btween */
	    $('input[name="dates"]').change(function() {
	    	let dates = $(this).val();
	    	$.ajax({
				  method: "GET",
				  url: "<?php echo base_url()?>Jobs/getbydateclose",
				  data: { date:dates },
				  dataType:'JSON',
